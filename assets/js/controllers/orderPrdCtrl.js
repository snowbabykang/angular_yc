(function() {
  'use strict';
    ...
    function selectOrderPrdCtl($scope, $uibModalInstance,item,$http,$modaltemp) {
		...
		$scope.showprdstore = function(item){
			...
			if(item.selectprd == true){
				...
				$scope.currenteditprd = item;  //更换商品去最后选中的那个商品
			}else{
		...
		//选择商品信息
		$scope.confirmAddPrdData = function(){
			if($scope.orderprdtype == "add"){
				var flag = 1;
				...
				var quantity = $scope.currenteditprd.quantity;
				var reg = /^[1-9][\d]*$/;
				if(!quantity || !reg.test(quantity)){
					$modaltemp.alertinfo("请填写正确的商品数量");
					return false;
				}else{
					$uibModalInstance.close($scope.currenteditprd);  
				}
			}
		};
		//填写商品数量时不选中当前项
	...
	//订单商品信息的controller
	operateOrderPrdCtl.$inject = ['$scope', '$uibModalInstance', 'item','$http','modaltemp.service','FileUploader'];
    function operateOrderPrdCtl($scope, $uibModalInstance,item,$http,$modaltemp,FileUploader) {
		$scope.orderprdtype = item.type;
