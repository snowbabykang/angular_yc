(function() {
  'use strict';
	...
	registerCtrl.$inject = ['$scope','$state','$http','$uibModal','modaltemp.service','FileUploader','$timeout'];
    function registerCtrl($scope,$state,$http,$uibModal,$modaltemp,FileUploader,$timeout) {
		...
		//请求导入订单列表操作
		var uploader1 = $scope.licenseuploader = new FileUploader({
            url: 'assets/js/data/upload.php?audioid=1',
            formData: [{
	    		 'timestamp': 1471420511  
	        }],
        });
		uploader1.onBeforeUploadItem = function(item) {
		    item.formData = [{'timestamp': 1471420511}];
		};
        ...
        uploader1.onSuccessItem = function(fileItem, response, status, headers) {
			...
			//将返回结果进行赋值，我在upload.php里面写死了返回值
			$scope.company.licenseImage = response;
			$timeout(function(){uploader1.queue = [];},1500);
        };
		var uploader2 = $scope.taxyRgNouploader = new FileUploader({
            url: 'assets/js/data/upload.php',
			formData: {   //举例请求参数
        		'timestamp': 1471420511  
            },
        });
        ...
        uploader2.onSuccessItem = function(fileItem, response, status, headers) {
            ...
            console.info(response);
			//将返回结果进行赋值，我在upload.php里面写死了返回值
			$scope.company.taxyRgNoImage = response;
			$timeout(function(){uploader2.queue = [];},1500);
        };
